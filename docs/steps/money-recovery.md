...
**弱の出方（2026-09-10）**: 同じ弱パターンは**ページ遷移（navigate）まで1回だけ**警告する（サイドメニューに
「プラン変更」が常在する管理画面で読み取りのたびに出て注意が薄れるのを防ぐ）。加えて、変更操作の**直前**に
ゲートが操作対象（要素名・ref・入力文字列）だけを照合し、弱なら【Money Watch・操作直前】の注意、
**強なら money_alert を立てて deny** する（座標クリックは文字列を持たないので照合されない — 画面読み取り側の検知が担う）。
