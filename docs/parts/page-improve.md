...
応募率・CV改善が目的の依頼では references/cro-jp/SKILL.md + references/psych-ux-jp/SKILL.md（視線・密度・配色・フォーム心理の判断表）を Read し、仮説（現状→変更→期待指標）を明示してから改善案を作る。効果検証の集計は outcome-verifier。
