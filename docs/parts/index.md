...
| クリエイティブ（画像・動画・ページ） | web-design + **psych-ux-jp**（心理根拠）+ **design-evidence-jp**（実証数値基準）+ 公開物は ad-compliance-jp 必須 | **ユーザーに渡す**ビジュアル成果物（モックアップ・カルーセル・バナー等）は **design-artisan**（生成）→ **design-critic**（審査）のループ。**critic の PASS を得るまでユーザーに引き渡さない**（REVISE なら FIX を artisan に戻して再審査（周回上限は agents/design-critic.md「収束条件」に従う）。依頼経路を問わずこの順序は省略不可）。**中間物は対象外** — 作業確認用スクショ・検証中のダミー・自分で見るだけの試作に critic を呼ばない（Critic Gate の対象スコープと同じ線引き） |
| 分析 | logical-writing / **seo-jp**（SEO診断）/ **cro-jp**（転換率のボトルネック分析） | 効果測定（返信率・エンゲージ集計）は **outcome-verifier**（集計対象の期間・件数があるときだけ。読み取り巡回の締めには呼ばない） |
| 掃き出し | —（不可逆送出の規律が規範） | 不可逆な一括送出は **pre-send-verifier**（敵対的監査）必須 → ユーザー承認 → 完了後に **outcome-verifier**（証跡検証）。**不可逆操作が無かったタスクではどちらも呼ばない** |
