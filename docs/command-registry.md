...
## 分類軸（2026-07-22 ユーザー指定で確定）

すべてを**事業ドメイン**で分類する。種類（調査/作る/運用）ではなくドメインが第一軸:
...
| 日本語名テスト | （実験） | 日本語スキル名の表示検証用。検証が済んだら削除 |

## 定常ループ台帳（標準セット。実運用はワークスペースのスケジュール登録が正）
...
**原則: 各ループの締めに `delve-dashboard` を実行**し、司令塔を常に最新へ。
