...
**動的パック**: 媒体の個別コマンド（例: /ワンキャリア /doda）は **/ワーク追加** で生成する — 登録 + 初期マッピング（フェーズ①・読み取り専用）+ ワークスペース `.claude/commands/` への専用コマンド生成を1コマンドで実行（プラグイン本体は増やさない）。
...
**計: 登録コマンド 14 / 内部手順 7 / 手順書 21（procedures/）**
