...
| 層 | 概念 | 実体 |
|---|---|---|
| **コマンド** | カテゴリーレベル（媒体・対象のタスクパック）。要望は引数に自由に書かせる | `commands/`（登録11本・日本語名）+ `procedures/delve-*.md`（内部手順含め27本） |
...
**動的パック**: 媒体の個別コマンド（例: /ワンキャリア /doda）は **/ワーク追加** で生成する — 登録 + 初期マッピング（フェーズ①・読み取り専用）+ ワークスペース `.claude/commands/` への専用コマンド生成を1コマンドで実行（プラグイン本体は増やさない）。**/セットアップ の媒体選択でも自動生成**する — SNS=`/<媒体名>運用`（/X運用 等）、広告媒体=`/<媒体名>広告`（/Google広告 等・参照先 delve-ads で媒体固定）、求人媒体=/ワーク追加接続で `/<媒体名>`。以後の単一媒体依頼は専用コマンドが第一入口（/SNS運用 /広告 /媒体管理 は複数媒体・媒体不明時の受け皿に降格）。**既存パックに該当しない完全別ワークは非表示の親「その他ワーク」の配下**に生成する（親判定表の正本は procedures/delve-add-work.md §4 — registry.yaml に `parent:` を記録）。
...
| delve-reporting | レポート | 記録 | core | 「今どうなってる？」「今日の作業まとめて」（トップ=状況サマリー + 作業ログ/運用レポートを選択） |
...
**計: 登録コマンド 11 / 内部手順 16 / 手順書 27（procedures/）**
...
| delve-demo | （内部）デモ | 「何ができるの？」→ commands/ の description から「できること一覧」を提示 |
...
|---|---|---|---|---|
| 自社・広告 | 自社サイト診断 | 週次（月曜） | Webサイト | アラート確認 |
...
**原則2: 各ループの締めにアラート確認を行う**（procedures/delve-reporting.md の状況サマリー部分。アラートがあればチャット/Slack で通知し、無ければ完了サマリーに「アラートなし」と1行添える）。旧キー `close_with_dashboard: true`（v1.16 以前の雛形）は本原則と同義に読む。
