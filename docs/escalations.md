...
|---|---|---|---|---|
| E1 | **最重要（セキュリティ）** | Credential Guard の ref すり抜け: hooks はツール引数の文字列しか見えず、`ref_150` 等の参照IDの解決先が type=password かを判定できない（2026-07-24 実測: Wikipedia 実 password 欄に入力が素通り） | docs/steps/credential.md（認証フィールドの取り扱い）の自己規律 + V5 に ref 回帰テスト + キーワード検知（従来） | **Claude in Chrome 拡張側で、入力系操作の ref 解決先要素の type=password / autocomplete（current-password / one-time-code 等）を検査してブロックする機構** |
...
## E4 の回避策候補（第3トポロジ・未実測・2026-07-27 起票）

**ホスト側 Claude Code + Claude in Chrome 拡張**という構成。ホスト（Windows）側で動くため
**plugin hooks が発火し、かつ Playwright / CDP も同居できる** — 「ゲート」と「決定論スクリプト（凍結）」を
両立できる唯一の構成になり得る。`hooks/hooks.json` の matcher は `mcp__claude-in-chrome__*` を登録済みなので、
ツール名が一致すればそのまま効く見込み。

**候補どまりであり、正式な回避策として台帳（上表）には書かない**（実測前の推測を設計前提に置かない — docs/rationale.md）。
正式化の前提となる実測3点:

1. **ツール名一致**: この経路で生えるツール名が `mcp__claude-in-chrome__*` と実際に一致するか
2. **hooks 発火ログ**: PreToolUse が実際に配線され deny が観測できるか（E4 の本体が解けるか）
3. **環境判定の解消**: `procedures/delve-start.md` 手順0.5 は `Bash` の有無で環境を判定するため、**この構成は cloud Cowork と区別できない**（どちらも `Bash` が生える）。ローカル分岐・凍結の主経路判断を誤るので判定材料の追加が必要
