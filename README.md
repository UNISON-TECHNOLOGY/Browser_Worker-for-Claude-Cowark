...
| /SNS運用 | 「Xの投稿ストック埋めて」「noteを書いて」（X/Instagram/TikTok/note/YouTube/LINE対応。媒体不明なら選択肢で確認） |
...
| 種類 | 内容 |
|---|---|
| コマンド 11（日本語・カテゴリーレベル） | /SNS運用 /リサーチ /媒体管理 /Webサイト /広告 + /セットアップ /ワーク追加 /定常タスク /ダッシュボード /作業ログ + /検証※開発用・配布時削除。台帳: docs/command-registry.md |
| 内部手順 13（メニュー非表示） | タスク開始（変更操作の関所）/ 状態確認 / デモ / 機能設定 / スキル化 / メモリ保存 / メモリ圧縮 + SNS媒体別手順6（X/Instagram/TikTok/note/YouTube/LINE — /SNS運用 が振り分け） |
| 手順書 24 | `procedures/delve-*.md` — 登録11+内部13 の手順の正本（メニューには登録されない。コマンド/ルールが Read して実行） |
