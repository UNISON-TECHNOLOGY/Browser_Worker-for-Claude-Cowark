...
| /タスク開始 /定常タスク /状態確認 | 変更操作の入口 / 定常化 / 「今どうなってる？」 |
| /ダッシュボード | 「ダッシュボード見せて」（運用状態+説明書+生成物リンク集を1枚に） |
| /作業ログ /スキル化 /メモリ保存 /メモリ圧縮 | 記録系: 報告書 / 手順の学習 / 評価の学習 / ログの構造化圧縮 |

はじめての方は `/デモ` でガイドツアーを体験できます。
...
| 種類 | 内容 |
|---|---|
| コマンド 20（日本語・カテゴリーレベル） | SNS媒体6（/X運用 /Instagram運用 /TikTok運用 /note運用 /YouTube運用 /LINE運用）+ /媒体管理 /Webサイト /広告 + 基盤6（/タスク開始 /定常タスク /状態確認 /ダッシュボード /デモ /機能設定）+ 記録5（/作業ログ /スキル化 /メモリ保存 /メモリ圧縮 /検証※開発用）。自然文から自動発火。台帳: docs/command-registry.md |
| 手順書 20 | `procedures/delve-*.md` — 各パックの手順の正本（メニューには登録されない。コマンドが Read して実行） |
