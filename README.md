...
| 種類 | 内容 |
|---|---|
| コマンド 13（日本語・カテゴリーレベル） | SNS媒体6（/X運用 /Instagram運用 /TikTok運用 /note運用 /YouTube運用 /LINE運用）+ /媒体管理 /Webサイト /広告 + /定常タスク /ダッシュボード /作業ログ + /検証※開発用・配布時削除。台帳: docs/command-registry.md |
| 内部手順 7（メニュー非表示） | タスク開始（変更操作の関所）/ 状態確認 / デモ / 機能設定 / スキル化 / メモリ保存 / メモリ圧縮 — 自然文・ルールで自動発火（procedures/ に実体） |
| 手順書 20 | `procedures/delve-*.md` — 登録13+内部7 の手順の正本（メニューには登録されない。コマンド/ルールが Read して実行） |
