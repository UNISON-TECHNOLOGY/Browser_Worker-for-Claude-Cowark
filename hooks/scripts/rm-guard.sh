...
MSG="【RM Guard】一括・再帰削除は機械ガード対象です。出口は2つ: (1) 自分が作成したファイルのパスを列挙し、個別に rm する（推奨。1コマンドに複数パスを並べるのは可、グロブ・-r は不可）。(2) どうしてもフォルダごと・グロブで消す必要がある場合は、実行しようとしたコマンドをそのままユーザーに提示し、ユーザー自身の手で実行してもらう。**AI 側で再試行・分割・別手段での回避を試みないこと**。削除できないまま終わる場合は、残置したパスを報告して完了してよい（削除の失敗はタスクの失敗ではない）。"
