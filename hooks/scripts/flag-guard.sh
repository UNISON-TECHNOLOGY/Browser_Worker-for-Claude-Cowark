...
MSG="【Flag Guard】memory/.workflow/ 配下のフラグはファイルツール（Write / Edit）では書けません。各手順が示す Bash コマンドで操作してください（k_done は touch memory/.workflow/k_done のみ。ov_done / critic_pass は手順どおり echo で記録）— OV Gate はそのコマンドを見て検証します。k_done は outcome-verifier の記録（ov_done）が揃ってから、psv_done は監査 + 承認が揃ってから。フラグだけ立てる迂回は禁止です。"
SHORT="【Flag Guard】memory/.workflow/ への Write/Edit は不可。Bash の touch / echo で（手順: procedures/delve-start.md）"
