...
# 【強】は抑制リストを見ずに判定する（2026-09-10 横断監査 C-1: AI が書き込めるワークスペースファイル1行で
# 金銭ガードの自動停止が無効化されないように。workflow-gate の操作直前判定と同じ順序）。
...
if [ -z "$matched" ]; then
  # 抑制リスト（【弱】専用の誤検知チューニング）: ユーザー本人が knowledge/config/money-suppress.txt に
  # 書いたパターンに当たるページは弱警告を出さない（例: 日常業務で開く媒体の管理画面URL/文言）
