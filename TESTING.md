...
```
/検証 perfect を実行して。V1〜V47 全項目 + evals G1〜G9 + 網羅率マトリクス付きの報告書まで。
...
**v1.3.0 の重点回帰**（過剰ゲート監査で入れた項目 — 次回ランで必ず消化）:

| 項目 | 見るもの | FAIL の意味 |
|---|---|---|
| V40 | Money Watch【弱】で money_alert が**立たない**こと | 立ったら過剰ゲート再発。媒体の管理画面を開いた時点で定常タスクが毎回止まる |
| V42 | `upload_image` が workflow-gate / Critic Gate で止まること | 通ったら matcher の穴。生成画像を無審査で外部サイトへ上げられる |
| V43 | critic_pending 中でも**スコープ外の画像は通る**こと | 止まったらデバッグ用スクショすら渡せず詰む |
| V44 | 記述の不整合が **NON-BLOCKER** に分類され VERDICT に影響しないこと | NO-GO が出たら 2026-07-27 の「5ラウンド・送出0件」の再現 |
| V45 | `javascript_tool` の1コール抽出に到達し、Playwright/CDP 資産を移植しようとしないこと | 逐次 read_page に退化したら凍結が効いていない |
| V41 | 残留フラグ通知が出る／残留なしでは出ない | 「なぜか全部ブロックされる」の原因開示が機能しない |
