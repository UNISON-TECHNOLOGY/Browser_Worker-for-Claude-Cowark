...
- **v1.18.0**: HTML レポートの deliverable-writer 委譲を廃止 — 作業ログ（delve-report）/ 媒体ステータスレポート（delve-media report）/ 改善提案レポート（parts/page-improve）/ スタイル比較レポート（parts/style-research）は、材料を集めた main ループが design-principles.md を読んで直接書き、conventions §2 で即アーティファクト発行する（委譲で買える独立性が無く、往復と絶対パス受け渡しのコストだけが残っていた）。deliverable-writer はテキスト成果物（求人票・スカウト・提案書・LP本文・記事）専用に。V9（deliverable-writer 起動確認）はテキスト委譲のままで有効。旧「Agent ツールが無い環境」のフォールバック規則は HTML 限定でなく汎用なので conventions §1.5 末尾へ移設（Opus レビュー I-1）。
- **v1.18.1**: RM Guard の誤爆修正（V39(b)）— ペイロード全文照合をやめ、tool_input.command だけを取り出し、クォート付きヒアドキュメント本文とシングルクォート内（bash -c / eval / xargs / パイプ先シェル等の再実行語が無い場合のみ）を照合から除外。未クォートのヒアドキュメントは $( ) / ` を含む行だけ残す。command が取れない入力は全文照合（fail-closed）。test-hooks に 11 件追加。 Opus レビュー反映: 抽出正規表現のエスケープ修正（`\n` を含む実ペイロードで全文照合に落ちていた）、クォート除去を出力系 allowlist に反転、空白なしクォート・`\rm` は常に剥がして照合、未終端ヒアドキュメントは本文を戻す fail-closed、awk/sed 空出力時は未加工照合、テストは実形式ペイロード。再レビュー2回目: クォート内・コメント内の << を開始扱いしない（終端行があると間の実コマンドが捨てられていた回帰）、allowlist 分類はクォート span を潰した写しで行う（クォート内の改行・;・& で文書化コマンドが誤爆）、危険パターンはクォート全削除の写しでも照合（rm' '-rf 分割）。再レビュー3回目: sed のクォート潰しが bash の字句規則とずれ、`"it's"` や `\'` を前後に置くと `;` ごと消えて rm が照合から外れる回帰 → クォート・エスケープ・コメント・ヒアドキュメント（同一行2本も可）を1本の awk 走査で追い、照合用 text / 分類用 mask / 文章除去用 strip の3写しを同じ走査（生コマンド）から作る方式に統一。閉じていないクォートは走査失敗として未加工で照合（fail-closed）。ダブルクォートの文章も出力系限定で除外。再レビュー4回目: 字句解釈のずれ3経路（`$'a\'b'` のエスケープ、`foo\ #x` のコメント誤認、`$((1 << x))` のヒアドキュメント誤認）と行継続（`\`+改行）を修正し、rm/find/git を argv 記録シムに差し替えて実行する差分ハーネス（bash が危険削除を実際に呼ぶコマンドは deny 必須、23件）を test-hooks に追加。10万文字超のコマンドは走査せず未加工で照合。再レビュー5回目: 先頭リダイレクト（`2>/dev/null bash -c '…'`）でセグメントごと読み飛ばされ bash -c / eval が無検査になっていた回帰 → リダイレクトと変数代入だけ剥がして残った先頭語を必ず検査。差分ハーネスにシム有効性の自己診断（無効なら FAIL）を追加。既知の穴（責務外・未対応）: 変数経由（`C=rm; $C -rf` / `cmd=$'rm'` など）、PowerShell の Invoke-Expression / ri -R 短縮 / .NET Directory.Delete、連結デリミタ `<<"EO"F` のヒアドキュメント（本文を通常行として照合＝安全側の誤爆）、PATH シムを経由しない `/bin/rm` 直接起動は差分ハーネスの守備範囲外。rm-guard 計57件＋差分ハーネス30件。
