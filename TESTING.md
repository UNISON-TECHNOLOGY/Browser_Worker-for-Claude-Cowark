...
- **v1.17.1**: RM Guard の誤爆修正（V39(b)）— ペイロード全文照合をやめ、tool_input.command だけを取り出し、クォート付きヒアドキュメント本文とシングルクォート内（bash -c / eval / xargs / パイプ先シェル等の再実行語が無い場合のみ）を照合から除外。未クォートのヒアドキュメントは $( ) / ` を含む行だけ残す。command が取れない入力は全文照合（fail-closed）。test-hooks に 11 件追加。
