...
- **v1.18.1**: RM Guard の誤爆修正（V39(b)）。照合対象をペイロード全文から tool_input.command に限定し、bash の字句規則（' / " / $'…' / \ / # / << / 行継続）を awk 1本で走査して、照合用 text（クォート付きヒアドキュメント本文とコメントを除外、空白なしクォートと \rm は剥がす）・分類用 mask・文章除去用 strip の3写しを作る。出力系のみ（echo / printf / cat / tee 等の連結で $( ) / ` / 先頭語の再実行語なし）のときだけ strip でクォート内の文章を落とす。解析失敗（未閉クォート・未終端ヒアドキュメント・command 抽出失敗・awk 不在・10万文字超）はすべて未加工で照合する fail-closed。test-hooks に静的 57 件と差分ハーネス（rm/find/git を argv 記録シムに差し替えて bash -c で実行し、危険削除が記録されたコマンドは deny 必須。シム有効性の自己診断付き）を追加。Opus レビュー 6 回（経緯は TESTING-archive.md「RM Guard v1.18.1 レビュー経緯」）。**既知の穴（責務外＝意図的な難読化。脅威モデルは「後片付けの拡大解釈で素直に打つ削除」）**: 変数経由（`C=rm; $C -rf` / `cmd=$'rm'`）、コマンド語自体のクォート（`'bash' '-c' 'rm -rf …'`）、PowerShell の Invoke-Expression / ri -R / .NET Directory.Delete、連結デリミタ `<<"EO"F`（安全側の誤爆）、PATH シムを経由しない `/bin/rm` は差分ハーネスの守備範囲外。**既存の誤爆（main から同じ）**: `git commit -m "… rm -rf …"` / `sed -i 's/rm -rf/…/' f` / `grep "rm -rf" f` は deny。コミットメッセージはクォート付きヒアドキュメントでファイルに書いて `git commit -F` で回避。
