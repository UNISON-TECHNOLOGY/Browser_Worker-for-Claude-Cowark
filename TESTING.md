...
- **v1.18.1**: RM Guard の誤爆修正（V39(b)）— ペイロード全文照合をやめ、tool_input.command だけを取り出し、クォート付きヒアドキュメント本文とシングルクォート内（bash -c / eval / xargs / パイプ先シェル等の再実行語が無い場合のみ）を照合から除外。未クォートのヒアドキュメントは $( ) / ` を含む行だけ残す。command が取れない入力は全文照合（fail-closed）。test-hooks に 11 件追加。 Opus レビュー反映: 抽出正規表現のエスケープ修正（
 を含む実ペイロードで全文照合に落ちていた）、クォート除去を出力系 allowlist に反転、空白なしクォート・
m は常に剥がして照合、未終端ヒアドキュメントは本文を戻す fail-closed、awk/sed 空出力時は未加工照合、テストは実形式ペイロード。再レビュー2回目: クォート内・コメント内の << を開始扱いしない（終端行があると間の実コマンドが捨てられていた回帰）、allowlist 分類はクォート span を潰した写しで行う（クォート内の改行・;・& で文書化コマンドが誤爆）、危険パターンはクォート全削除の写しでも照合（rm' '-rf 分割）。既知の穴（責務外・未対応）: 変数経由 `C=rm; $C -rf`、PowerShell の Invoke-Expression / ri -R 短縮 / .NET Directory.Delete。rm-guard 計38件。
