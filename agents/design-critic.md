...
同一の成果物について、**審査は最大2周**とする（呼び出し側の手順書はこの節をポインタ参照する。周回上限をそちらに書かない）。**目視審査モード（スクリーンショット審査）は全体の2周とは別枠で1回のみ**（差分審査に相当するため増やさない）。1回で REVISE が残る場合は修正後に再目視せず `VERDICT: HUMAN-REVIEW-REQUIRED` を返す。
...
スクリーンショット画像が渡された場合は、コードではなくレンダリング結果を審査する: 要素の重なり・はみ出し・コントラスト不足・余白の破綻・モバイル幅での崩れ。応答形式は同じ。周回上限は「収束条件」に従う（別枠で1回のみ）。
