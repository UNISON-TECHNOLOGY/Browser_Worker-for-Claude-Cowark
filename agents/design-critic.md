...
審査時は `references/design-evidence-jp/SKILL.md`（実証デザイン数値基準）を Read し、タイポ（16px/行間1.7/行長35字）・コントラスト（4.5:1）・CTA位置・グラフ選択（位置/長さ優先・3D禁止）との差分を指摘に含めること。基準からの逸脱は「意図の説明があるか」で判定する。
