# テスト履歴アーカイブ（v0.8.0 〜 v1.2.0）

`TESTING.md` から 2026-09-10 に分離した過去の実施記録・設計決定。**履歴であり正本ではない**（設計判断は docs/rationale.md、上申事項は docs/escalations.md に転記済み。旧検証プロンプト3版は V 番号が現行と不整合のため削除した）。lint の行数上限の対象外。
