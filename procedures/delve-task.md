...
- `knowledge/config/loops.yaml` に追記（なければ作成）:
  ...
  ```
- **スケジュール登録 — 実行環境の一致が最優先**: **ブラウザ操作を伴うタスクを `create_trigger`（クラウド発火）で登録してはいけない**。クラウドの新規セッションは実PCのログイン済み Chrome に届かず、トリガーは発火するのに実行不能で空振りする（2026-07-24 実測: IGトリガー 10:07 発火→実行不達）。ブラウザ操作タスクはローカル（デスクトップアプリの「このコンピュータで実行」スケジュール）で登録し、`create_trigger` はブラウザ操作を伴わないタスク（集計・レポート生成・ファイル処理等）のみに使う。**ローカル登録の前に「無人運用前チェック」（docs/unattended-ops.md — 優先ブラウザの媒体別ログイン状態を読み取り巡回で確認）を実行し、✗ 媒体があればオーナーのログイン揃えを先に依頼する。**使えなければ登録文を提示してユーザーに委ねる。登録規則（cron の UTC 評価・最小間隔・ジッター・発火プロンプト書式）の**正本は `docs/unattended-ops.md`** — 必ず Read してそれに従う（ここには複製しない）
