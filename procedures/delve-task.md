...
- `knowledge/config/loops.yaml` に追記（なければ作成）:
  ...
  ```
- **スケジュール登録**: trigger 系ツール（`create_trigger`）が使える環境なら、ユーザーに周期を確認のうえ直接登録してよい。使えなければ登録文を提示してユーザーに委ねる。登録規則（cron の UTC 評価・最小間隔・ジッター・発火プロンプト書式）の**正本は `docs/unattended-ops.md`** — 必ず Read してそれに従う（ここには複製しない）
