...
| V30 | 動的コマンド生成 | (a) /ワーク追加 をダミー媒体（example.com 管理画面想定）でドライラン（マッピングは1ページのみ・登録後に削除） (b) delve-setup の媒体選択経由で SNS 専用コマンド（例: /X運用）の生成をドライラン（生成物確認後に削除。setup.yaml は元に戻す） | (a) .claude/commands/<媒体>.md が規約どおり生成され、削除フローで消える (b) 専用コマンドが生成され、command-registry.md の「4点配線」チェックリストの体裁を満たす |
